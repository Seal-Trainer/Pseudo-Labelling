...
dataset <- read.csv(file = "Data/citrus.csv")
...
temp$name <- factor(temp$name, labels = c(0,1))

predictor <- temp[ , 2:5]
predictor <- as.matrix(predictor)
variableRespuesta <- temp[ , 1]
variableRespuestaFES <- temp[ , 1]
...
for(i in 1:nIteracciones)
{
  ...
  print(i)
  ## Falso Etiquedado = FE
  indicesEntrenamiento <- sample(nFilas, nFilas/2)
  indicesTesteo <- setdiff(1:nFilas, indicesEntrenamiento)
  indicesTesteo <- sample(indicesTesteo, nFilas/4)
  indicesFE <- setdiff(1:nFilas, c(indicesEntrenamiento, 
                                   indicesTesteo))
  indicesFE <- sample(indicesFE, nFilas/4)                                              
...
write.csv(resultados, file = "Resultados/resultadosCompletos2.csv")
...
write.csv(resumen, file = "Resultados/resumenPorParticion2.csv")
...
write.csv(resumen, file = "Resultados/resumen2.csv")